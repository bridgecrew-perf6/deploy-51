<template>
  <div class="d-flex">
    ...
    <div class="container create-gt__container">
      ...
      <template v-if="event.training && event.training.id">
        ...
        <training-plan-part
          @added="createTrainingExercise"
          @removed="deleteTrainingExercise"
          type="preparation"
...
export default {
  ...
  data() {
    return {
      ...
      training: {
        ...
        objectType: this.$route.query.objectType || this.event.group ? 'group' : 'sportsman',
        trainingType: 'physical',
        placeType: this.$route.query.placeType || 0,
      },
  ...
  computed: {
    pageTitle() {
      if (this.event.group) return 'Редактирование группового занятия'
      else return 'Редактирование индивидуального занятия'
    },
  },
  watch: {
    event: {
  ...
  methods: {
    ...
    diffEvent() {
      ...
      if (!_.isEmpty(newData)) {
        if (_.has(_.fromPairs(newData), 'training')) this.updateTraining()
    ...
    updateTraining() {
      let newDataTraining = _.differenceWith(
        _.toPairs(this.event.training),
        _.toPairs(this.copyEvent.training),
        _.isEqual
      )

      if (!_.isEmpty(newDataTraining)) {
        this.$events.updateTraining(this.event.id, this.event.training.id, _.fromPairs(newDataTraining)).then(() => {
          this.copyObject()
          this.notify({
            title: 'Успешно',
            text: `Событие успешно обновлено`,
      ...
      this.$events.getTrainingExercises(this.event.id, this.event.training.id).then(response => {
        ...
        if (exercisesListID) {
          this.$exercises.getExercises({ id__in: exercisesListID }).then(exercisesList => {
            ...
            this.exercisesTraining = response
          })
        }
      })
    },
    createTrainingExercise(ex, stage) {
      let payload = {
        trainingStage: stage,
        exercise: ex.id,
      }
      this.$events.createTrainingExercises(this.event.id, this.event.training.id, payload).then(response => {
        response.exercise = ex
        this.exercisesTraining.push(response)
    ...
    deleteTrainingExercise(ex) {
      this.$events.deleteTrainingExercises(this.event.id, this.event.training.id, ex.element.id).then(() => {
        this.exercisesTraining = _.reject(
          this.exercisesTraining,
          this.exercisesTraining.find(e => e.id === ex.element.id)
        )
