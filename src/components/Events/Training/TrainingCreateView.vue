  ...
  <div class="d-flex">
    <left-menu>
      <training-sidebar-sportsman
        v-if="this.$route.query.objectType === 'individual' || newEvent.training.objectType === 'individual'"
        :idSportsman="this.$route.query.sportsman || newEvent.sportsman"
      />
      <training-sidebar-group
        edit
        @changeGroup="changeGroup"
        v-else-if="this.$route.query.objectType === 'group' || newEvent.training.objectType === 'group'"
    ...
    <div class="container create-gt__container">
      ...
      <div class="create-gt__header d-flex justify-space-between align-center">
        <h1 class="create-gt__title h1">{{ pageTitle }}</h1>
      ...
      <training-plan-part
        @added="addTrainingExercise"
        @removed="removeTrainingExercise"
        type="preparation"
        :exercises="exercises"
      />
      <training-plan-part
...
export default {
  ...
  data() {
    return {
      ...
      newEvent: {
        ...
        training: {
          objectiveType: '',
          objectType: this.$route.query.objectType || null,
          trainingType: '',
  ...
  computed: {
    pageTitle() {
      switch (this.newEvent.training.objectType) {
        case 'group':
          return 'Создание группового занятия'
        case 'individual':
          return 'Создание индивидуального занятия'
        default:
          return ''
      }
    },
  ...
  methods: {
    ...
    createEvent() {
      ...
      if (this.$route.name === 'TrainingCopyView') {
      ...
      this.$events
        .postEvents(payload)
        .then(response => {
          this.createTraining(response.id)
          this.notify({
    ...
    createTraining(idEvent) {
      this.$events
        .createTraining(idEvent, this.newEvent.training)
        .then(response => {
          this.notify({
            title: 'Тренировка успешно создана',
            text: `Тренировка #${response.id} создана`,
            bg: '#64C048',
            autoClose: 4000,
          })

          let idTraining = response.id

          this.addExercises(idEvent, idTraining)
        })
        .catch(error => {
          this.notify({
            title: 'Ошибка',
    ...
    addExercises(idEvent, idTraining) {
      let promises = []

      this.exercises.forEach(exercise => {
        promises.push(
          this.addExercise(idEvent, idTraining, {
            trainingStage: exercise.trainingStage,
      ...
      Promise.all(promises)
        .then(response => {
          this.notify({
            ...
            text: `Тренировка полностью создана`,
            bg: '#64C048',
            autoClose: 4000,
          })
    ...
    addExercise(idEvent, idTraining, payload) {
      return this.$events.createTrainingExercises(idEvent, idTraining, payload)
    },
    addTrainingExercise(ex, stage) {
      this.exercises.push({
        trainingStage: stage,
        exercise: ex,
      })
    },
    removeTrainingExercise(ex) {
      this.exercises = _.reject(
        this.exercises,
        this.exercises.find(e => e === ex.element && e.trainingStage === ex.element.trainingStage)
      )
    },
    copyInfo() {
      let dateTime = {
      ...
      this.getExercisesForCopy(this.event.id, this.event.training.id)
    },
    getExercisesForCopy(idEvent, idTraining) {
      this.$events.getTrainingExercises(idEvent, idTraining).then(response => {
  ...
  validations: {
    ...
    exercises: {
      preparationRequired() {
        return this.exercises.filter(ex => ex.trainingStage === 'preparation').length > 0 ? true : false
      },
      mainRequired() {
        return this.exercises.filter(ex => ex.trainingStage === 'main').length > 0 ? true : false
      },
      finalRequired() {
        return this.exercises.filter(ex => ex.trainingStage === 'final').length > 0 ? true : false
      },
    },
  },
  mounted() {
    if (this.$route.name === 'TrainingCopyView') this.copyInfo()
  },
}
</script>
