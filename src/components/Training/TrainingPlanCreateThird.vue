<template>
  <div class="training-plan training-plan--third">
    <div class="training-plan__title-wrapper">
      <span class="training-plan__title-icon"></span>
      <p class="training-plan__title">Часть III - Заключительная</p>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters('events', ['getUtils', 'getThirdExercises']),
    utils() {
      return this.getUtils
    },
    library() {
      return this.getThirdExercises
    },
  ...
  watch: {
    filter: {
      handler: _.debounce(function (newValue) {
        this.loadThirdExercises(newValue)
      }, 500),
      deep: true,
    },
    selectedExercises: {
  ...
  methods: {
    ...mapActions('events', ['loadUtils', 'loadThirdExercises']),
    choose(evt) {
      this.trash = true
    },
    unchoose(evt) {
