<template>
  <div :class="`training-plan training-plan--${typePart.id}`">
    <div class="training-plan__title-wrapper">
      <span class="training-plan__title-icon"></span>
      <p class="training-plan__title">Часть {{ typePart.romanNum }} - {{ typePart.name }}</p>
    </div>
          ...
          <vue-custom-scrollbar class="library__exercise-list-scroll" :settings="scrollSettings" style="height: 100%">
            <v-expansion-panels>
              <draggable
                ...
                group="exercises"
                :disabled="exercisesList.length < 2"
                :list="exercisesList"
                @choose="choose"
                @unchoose="unchoose"
                @change="change"
              >
                <v-expansion-panel
                  ...
                  v-for="(element, index) in exercisesList"
                  :key="index"
                >
                  <v-expansion-panel-header class="training-plan__exercises-collapse-header">
                    <span class="training-plan__exercises-collapse-title">{{ element.exercise.title }}</span>
                    <p class="training-plan__exercises-collapse-time-wrap">
                      <span class="training-plan__exercises-collapse-time">{{
...
export default {
  ...
  computed: {
    ...mapGetters('events', ['getIntensityList', 'getTrainingStage']),
    exercisesList() {
      return this.exercises.filter(ex => ex.trainingStage === this.type || ex.standardStage === this.type)
    },
    timeSelectedExercises() {
      try {
        return this.exercisesList.reduce((sum, current) => sum + parseInt(current.exercise.duration || 0), 0)
      } catch (error) {
        return 0
      }
    },
    intensityList() {
      return this.getIntensityList
    },
    trainingStage() {
      return this.getTrainingStage
    },
    typePart() {
      let type = this.trainingStage.find(stage => stage.id === this.type),
        empty = {
          id: '',
          name: '',
          romanNum: '',
  ...
  methods: {
    loadExercises(params) {
      this.$exercises.getExercises(params || this.filter).then(response => {
        response.forEach(ex => {
          ex.duration = parseInt(ex.duration || 0)
        })

        this.library = response.filter(ex => ex.preparationPeriods.indexOf(this.type) !== -1)
        this.load = true
      })
    },
    choose(evt) {
    ...
    addExercise(el) {
      this.$emit('added', el, this.type)
    },
    change({ added, removed }) {
      if (added) this.$emit('added', added, this.type)
...
.training-plan--testing {
  .training-plan__title-icon {
    ...
    &::after {
      background-image: url("data:image/svg+xml,%3Csvg width='18' height='20' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cg clip-path='url(%23a)' fill='%23fff'%3E%3Cpath d='M13.77 4.6c.399-.46.775-.906 1.162-1.342.424-.476.988-.546 1.402-.186.448.388.476.897.056 1.41-.394.478-.805.94-1.24 1.445 1.79 2.27 2.394 4.828 1.706 7.626-.509 2.068-1.698 3.712-3.453 4.932-3.523 2.45-8.288 1.876-11.224-1.348C-.623 14.06-.73 9.245 1.899 6.029 4.569 2.76 9.571 1.589 13.77 4.6ZM9.499 8.093V6.87l-.001-.107c-.031-.666-.378-1.05-.944-1.046-.566.003-.915.385-.928 1.057-.014.76-.02 1.523.002 2.283.012.424-.064.74-.414 1.058-.757.688-.734 1.89-.03 2.625a1.925 1.925 0 0 0 2.638.102c.765-.68.904-1.892.174-2.609-.456-.446-.533-.898-.498-1.453.014-.228.002-.459 0-.69ZM8.592.003c.619 0 1.237-.01 1.856.003.577.012.946.372.96.909.016.532-.336.971-.893.984-1.307.03-2.617.03-3.924 0-.558-.015-.898-.455-.878-.996.02-.538.384-.886.97-.897.636-.011 1.273-.003 1.91-.003Z'/%3E%3C/g%3E%3Cdefs%3E%3CclipPath id='a'%3E%3Cpath fill='%23fff' d='M0 0h17.134v20H0z'/%3E%3C/clipPath%3E%3C/defs%3E%3C/svg%3E");
    }
  }
