<template>
  <div class="training-plan training-plan--second" :class="[{ 'training-plan--only': trainingOver }]">
    <div class="training-plan__title-wrapper" :class="[{ 'training-plan__title-wrapper-red': !trainingOver }]">
      <span class="training-plan__title-icon" :class="[{ 'training-plan__title-icon-red': !trainingOver },
                                                       { 'training-plan__title-icon-one': trainingOver} ]"></span>
      <p v-if="!trainingOver" class="training-plan__title">Часть II - Тестирование</p>
      <p v-if="trainingOver" class="training-plan__title">Часть I</p>
    </div>
    <div class="training-plan__content">
      <div v-if="!trainingOver" class="training-plan__content-header">
      ...
      <div class="training-plan__content-body">
        <div v-if="trainingOver" class="training-plan__content-header">Часть II - тестирование</div>
        <div
          class="training-plan__library-wrapper"
          :class="[{ 'training-plan__library-wrapper--empty': !library.length },
                    {'training-plan__library-wrapper--short': trainingOver}]"
        >
        ...
        <div class="training-plan__exercises-wrapper" v-if="!trainingOver">
    ...
    <div v-if="trainingOver" class="training-plan__title-wrapper-three">
      <span class="training-plan__title-icon training-plan__title-icon-three"></span>
      <p class="training-plan__title-three">Часть III</p>
    </div>
  </div>
</template>
...
export default {
  props: {
    trainingOver: { type: Boolean, default: false },
  },
  data() {
    return {
      trash: false,
  ...
  computed: {
    ...mapGetters('events', ['getUtils', 'getSecondExercises']),
    utils() {
      return this.getUtils
    },
    library() {
      return this.getSecondExercises
    },
  ...
  watch: {
    filter: {
      handler: _.debounce(function (newValue) {
        this.loadSecondExercises(newValue)
      }, 500),
      deep: true,
    },
    selectedExercises: {
  ...
  methods: {
    ...mapActions('events', ['loadUtils', 'loadSecondExercises']),
    choose(evt) {
      this.trash = true
    },
    unchoose(evt) {
...
.training-plan__title-wrapper-red {
  background-color: #FF4B6B;
}

.training-plan__title-wrapper-three {
  height: 100%;
  width: 64px;
}
...
.training-plan__title-icon-red {
  background: linear-gradient(0deg, rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.2)), #FF4B6B;
}
...
.training-plan__library-wrapper--short {
