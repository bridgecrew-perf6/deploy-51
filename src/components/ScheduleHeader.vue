  ...
  <div class="schedule-header">
    <v-row justify="space-between">
      <v-col class="d-flex align-center" md="6">
        ...
        <wu-select-period v-model="per" @change="changeView" />
...
import BaseBadge from './library/BaseBadge.vue'
import ModalTrainerNewEvent from '@/components/modals/ModalTrainerNewEvent.vue'
export default {
  components: { BaseBadge, ModalTrainerNewEvent },
  name: 'ScheduleHeader',
  props: {
    title: {
      type: String,
  ...
  computed: {
    ...
    selectedYear() {
      return moment(this.initDay).startOf('year').format('YYYY')
    },
  },
  watch: {
    '$route.name': {
