<template>
  <v-dialog v-model="show" :max-width="maxWidth">
    ...
    <v-card class="modal-reminders">
      ...
      <v-row>
        <v-col :cols="model < 2 ? 12 : 6" class="reminders-slider__wrapper">
        ...
        <v-col class="reminders-slider__wrapper reminders-slider__wrapper--padding" v-if="model >= 2">
          ...
          <template v-if="model === 3">
            ...
            <v-text-field
              class="modal-create__input modal-create__input--small modal-create__input--year"
              height="auto"
              flat
              hide-details
              placeholder="01/12"
              v-model="form.year"
              :rules="[rules.annually]"
            ></v-text-field>
...
export default {
  ...
  data() {
    return {
      ...
      selectionDays: [],
      rules: {
        monthly: day => (day >= 1 && day <= 31) || '',
        annually: date => this.$moment(date, 'DD/MM').isValid() || '',
        time: time => this.$moment(time, 'hh:mm').isValid() || '',
      },
      form: {
        repeat: '',
        month: '',
...
.modal-create__input--year
  max-width: 60px

.reminders-slider__error
  font-weight: 300;
  font-size: 10px;
  line-height: 14px;
  color: $terrible;
