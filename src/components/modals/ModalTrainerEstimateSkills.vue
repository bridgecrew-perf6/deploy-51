...
export default {
  ...
  props: ['visible', 'sportsman', 'exer', 'plan', 'mistakelist', 'mistakeFactorList', 'profile'],
  computed: {
    showSkills: {
      get() {
        return this.visible
    ...
    estimate() {
      ...
      elems.map(item => {
        if (item.exercises_estimate !== null) {
          if (item.exercises_estimate + 1 < 7 && item.$refs.home) {
            item.$refs.home.homemistakes.map((ms, index) => {
              if (ms.mistakes.length == 0) {
                error = true
                Vue.set(item.$refs.home.errorMistake, index, true)
              }

              if (ms.factors.length == 0) {
                error = true
                Vue.set(item.$refs.home.errorFactor, index, true)
              }
            })
          }
        }
...
.exercises-estimate {
  ::v-deep {
    .v-btn {
      ...
      border: none;
      margin-right: 8px;
      @include width-flex(60px) height: 40px !important;
      font-family: $FiraSansMedium;
      font-size: 16px;
      background: #f1f3f9;
      border-radius: 2px;
...
.estimate-tooltip-num {
  ...
  @include width-flex(36px) height: 36px;
  display: flex;
  align-items: center;
...
.selected-estimate__num {
  color: #326bff;
  font-family: $FiraSansBold;
  @include width-flex(24px) height: 24px;
  display: flex;
  font-size: 14px;
  align-items: center;
  justify-content: center;
