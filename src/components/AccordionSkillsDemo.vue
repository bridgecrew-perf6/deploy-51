  ...
  v-expansion-panels.acc-exercises__panels(v-model="panel" multiple)
    v-expansion-panel.acc-exercises__panel
      ...
      v-expansion-panel-content.acc-exercises__content
        .mistake__block.mb-3(v-for="(homemistake, index) in homemistakes")
          .mistake__block-left
          .mistake__block-right
            .mistake__row.mb-4
              base-select(
                ...
                label="Выберите"
                :items="mistakes"
                :error="errorMistake[index]"
                v-model="homemistake.mistakes"
                @change="errorMistake[index]=false"
              )
            ...
            .mistake__row.mb-3
              base-select(
                ...
                :multiple="true"
                :error="errorFactor[index]"
                @change="errorFactor[index]=false"
              )
...
.mistake__row {
  display: flex;
  align-items: center;
  @include width-flex(100%) .select-default {
