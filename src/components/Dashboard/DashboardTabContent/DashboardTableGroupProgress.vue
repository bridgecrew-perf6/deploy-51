<template>
  ...
  >
    ...
    <template v-slot:header.progress.ofp="{ header }">
      <div class="progress">
        <p class="progress__value">{{ header.percent }}<span class="progress__percent">%</span></p>
        <span class="progress__title">{{ header.text }}</span>
      </div>
      <v-progress-linear
        class="progress__line"
        background-color="#36427d"
        :color="getHexColor(header.percent)"
        :value="header.percent"
        height="5"
      ></v-progress-linear>
    </template>

    <template v-slot:header.progress.theory="{ header }">
      <div class="progress">
        <p class="progress__value">{{ header.percent }}<span class="progress__percent">%</span></p>
        <span class="progress__title">{{ header.text }}</span>
      </div>
    ...
    <template v-slot:header.progress.tactic="{ header }">
      <div class="progress">
        <p class="progress__value">{{ header.percent }}<span class="progress__percent">%</span></p>
        <span class="progress__title">{{ header.text }}</span>
      </div>
    ...
    <template v-slot:header.progress.technique="{ header }">
      <div class="progress">
        <p class="progress__value">{{ header.percent }}<span class="progress__percent">%</span></p>
        <span class="progress__title">{{ header.text }}</span>
      </div>
    ...
    <template v-slot:header.progress.psychology="{ header }">
      <div class="progress">
        <p class="progress__value">{{ header.percent }}<span class="progress__percent">%</span></p>
        <span class="progress__title">{{ header.text }}</span>
      </div>
    ...
    <template v-slot:item.name="{ item }">
      <div class="player">
        ...
        <div class="player__about">
          ...
          <div class="player__indicators">
            <p class="player__mood">
              Настроение
              <span class="player__mood-value">
                <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M4.93834 5.62767C4.93834 5.1203 4.52122 4.70117 4.01629 4.70117C3.51136 4.70117 3.09424 5.1203 3.09424 5.62767C3.09424 6.11298 3.51136 6.5321 4.01629 6.5321C4.52122 6.5321 4.93834 6.11298 4.93834 5.62767Z"
                    fill="white"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M7.94598 4.70117C7.44105 4.70117 7.0459 5.1203 7.0459 5.62767C7.0459 6.11298 7.44105 6.5321 7.94598 6.5321C8.47285 6.5321 8.86801 6.11298 8.86801 5.62767C8.88996 5.1203 8.47285 4.70117 7.94598 4.70117Z"
                    fill="white"
                  />
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.01426 0.972656C2.72127 0.972656 0.0429688 3.66383 0.0429688 6.97266C0.0429688 10.2815 2.72127 12.9727 6.01426 12.9727C9.30725 12.9727 11.9855 10.2815 11.9855 6.97266C11.9855 3.66383 9.30725 0.972656 6.01426 0.972656ZM6.01426 12.4874C2.98471 12.4874 0.503987 10.0168 0.503987 6.9506C0.503987 3.90648 2.96275 1.41383 6.01426 1.41383C9.04381 1.41383 11.5245 3.88442 11.5245 6.9506C11.5245 10.0388 9.04381 12.4874 6.01426 12.4874Z"
                    fill="white"
                  />
                  <path d="M8.14383 8.98047H4.0166V9.44366H8.14383V8.98047Z" fill="white" />
                </svg>
              </span>
            </p>
            <p class="player__stress">Стресс <span class="player__stress-value">2</span></p>
            <p class="player__pain">Боль <span class="player__pain-value">5</span></p>
          </div>
        </div>
      </div>
    </template>

    <template v-slot:item.progress.ofp="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.progress.ofp)" dark>
        {{ item.progress.ofp }}
        <span class="db-tab-chip__percent">%</span>
      </v-chip>
    </template>
    <template v-slot:item.progress.theory="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.progress.theory)" dark>
        {{ item.progress.theory }}
        <span class="db-tab-chip__percent">%</span>
      </v-chip>
    </template>
    <template v-slot:item.progress.tactic="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.progress.tactic)" dark>
        {{ item.progress.tactic }}
        <span class="db-tab-chip__percent">%</span>
      </v-chip>
    </template>
    <template v-slot:item.progress.technique="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.progress.technique)" dark>
        {{ item.progress.technique }}
        <span class="db-tab-chip__percent">%</span>
      </v-chip>
    </template>
    <template v-slot:item.progress.psychology="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.progress.psychology)" dark>
        {{ item.progress.psychology }}
        <span class="db-tab-chip__percent">%</span>
      </v-chip>
    </template>
  </v-data-table>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'ОФП', value: 'progress.ofp', percent: 74, align: 'center', width: '12.72%' },
        { text: 'Теория', value: 'progress.theory', percent: 54, align: 'center', width: '12.72%' },
        { text: 'Тактика', value: 'progress.tactic', percent: 36, align: 'center', width: '12.72%' },
        { text: 'Техника', value: 'progress.technique', percent: 12, align: 'center', width: '12.72%' },
        { text: 'Психология', value: 'progress.psychology', percent: 56, align: 'center', width: '12.72%' },
      ],
    }
  },
  computed: {
      ...
      for (let index = 0; index < 20; index++) {
        let element = {
          ...
          progress: {
            ...
            theory: Math.floor(Math.random() * (100 - 0 + 1) + 0),
            tactic: Math.floor(Math.random() * (100 - 0 + 1) + 0),
            technique: Math.floor(Math.random() * (30 - 0 + 1) + 0),
            psychology: Math.floor(Math.random() * (50 - 0 + 1) + 0),
          },
        }
        arr.push(element)
      }
