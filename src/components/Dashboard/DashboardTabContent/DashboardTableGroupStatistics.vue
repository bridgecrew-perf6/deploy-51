<template>
  ...
  >
    ...
    <template v-slot:item.stats.games="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.stats.games)" dark>
        {{ item.stats.games }}
      </v-chip>
    </template>
    <template v-slot:item.stats.score="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.stats.score)" dark>
        {{ item.stats.score }}
      </v-chip>
    </template>
    <template v-slot:item.stats.passes="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.stats.passes)" dark>
        {{ item.stats.passes }}
      </v-chip>
    </template>
    <template v-slot:item.stats.goals="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.stats.goals)" dark>
        {{ item.stats.goals }}
      </v-chip>
    </template>
    <template v-slot:item.stats.time="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.stats.time)" dark>
        {{ item.stats.time }}
      </v-chip>
    </template>
    <template v-slot:item.stats.plus="{ item }">
      <v-chip class="db-tab-chip" :color="getColor(item.stats.plus)" dark>
        {{ item.stats.plus }}
      </v-chip>
    </template>
  </v-data-table>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Кол-во игр', value: 'stats.games', align: 'center', width: '10.6%' },
        { text: 'Очки', value: 'stats.score', align: 'center', width: '10.6%' },
        { text: 'Передачи', value: 'stats.passes', align: 'center', width: '10.6%' },
        { text: 'Голы', value: 'stats.goals', align: 'center', width: '10.6%' },
        { text: 'Штраф.время', value: 'stats.time', align: 'center', width: '10.6%' },
        { text: '+/-', value: 'stats.plus', align: 'center', width: '10.6%' },
      ],
    }
  },
  computed: {
    sportsmen() {
      ...
      for (let index = 0; index < 20; index++) {
        let element = {
          ...
          avatar: null,
          stats: {
            games: Math.floor(Math.random() * (100 - 0 + 1) + 0),
            score: Math.floor(Math.random() * (100 - 0 + 1) + 0),
            passes: Math.floor(Math.random() * (100 - 0 + 1) + 0),
            goals: Math.floor(Math.random() * (30 - 0 + 1) + 0),
            time: Math.floor(Math.random() * (50 - 0 + 1) + 0),
            plus: Math.floor(Math.random() * (10 - -10 + 1) + -10),
          },
        }
        arr.push(element)
      }
