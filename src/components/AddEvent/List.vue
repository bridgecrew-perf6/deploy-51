<template>
  <div class="list">
    ...
    <div
      class="list__wrapper"
      :class="{
        'list__wrapper_active-stages': showForm && type === 'stages',
        'list__wrapper_active-doc': showForm && type === 'doc',
      }"
    >
      <vue-custom-scrollbar class="list__scroll custom-scroll" :settings="scrollSettings">
        <div class="list__list">
          ...
          <div class="list__item" v-show="form.items.length" v-for="(item, index) in form.items" :key="index">
            <div class="list__item-content">
              <div
                class="list__item-icon"
                :style="{ backgroundImage: `url(${require('@/assets/images/svg/dots.svg')})` }"
              ></div>
        ...
        <div class="list__form-wrapper">
          <div class="list__form-title-inp">
            <div
              ...
              :style="{ backgroundImage: `url(${require('@/assets/images/svg/dots.svg')})` }"
            ></div>
            <input type="text" placeholder="Введите название" v-if="type === 'stages'" v-model="form.form.title" />
            <input
              type="text"
          ...
          <button class="list__form-save list__form-save_stages" @click="addItem" v-if="type === 'stages'">
        ...
        <div class="list__form-wrapper" v-if="type === 'doc'">
          <div class="list__form-inps">
            <div class="inp-wrapper list__form-max-inp">
              <p>Срок исполнения </p>
              <wu-input-date v-model="form.form.date" class="team-selection__inp-date" />
            </div>
...
export default {
  ...
  watch: {
    documents: {
      handler() {
        if (this.type !== 'doc') return
        if (this.documents.length) this.form.items = this.documents
      },
      deep: true,
    },
    preparationStages: {
      handler() {
        if (this.type !== 'stages') return
        if (this.preparationStages.length) this.form.items = this.preparationStages
      },
      deep: true,
    },
  },
  methods: {
    addItem() {
      ...
      this.showForm = false
      this.$emit('change', this.form.items)
    },
    deleteItem(index) {
      if (this.form.items[index].id) this.$emit('delete', this.form.items[index].id)
