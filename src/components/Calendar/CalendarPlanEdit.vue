                ...
                v-col(
                  ...
                  AccordionBig(
                    ...
                    template(v-slot:accord-body)
                      TrainingAccordion(
                        :groupt="planEx[accordionBigItem.slug]"
                        :groupName="accordionBigItem.slug"
                        :utils="utils"
                        :events="accordionBigItem.exercisesEvent"
                        @deleteTrainig="deleteTraining"
                      )
              v-row
                v-col(
                  md='6'
...
export default {
  ...
  methods: {
    deleteTraining: function (index, slug) {
      this.planEx[slug].splice(index, 1)
    },
    add: function () {
  ...
  created() {
    let pre = this.plan.eventParts.filter(item => item.slug == 'pre')
    this.planEx.pre = this.getExBigItem(pre[0].exercisesEvent)

    let main = this.plan.eventParts.filter(item => item.slug == 'main')
    this.planEx.main = this.getExBigItem(main[0].exercisesEvent)

    let end = this.plan.eventParts.filter(item => item.slug == 'end')
    this.planEx.end = this.getExBigItem(end[0].exercisesEvent)
