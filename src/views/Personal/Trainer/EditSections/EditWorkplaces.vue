        ...
        .place__block(v-for="(form, idx) in forms" :key="idx")
          .inputs-row
            ...
            .profile-table__right
              v-row.align-center
                v-col(md='4')
                  DataPicker(:value="form.dateStart" :min-date="false" @input="(v) => handleDatepickerChange(v, idx, 'dateStart')")
                v-col(md='4')
                  .profile-title Дата окончания
                v-col(md='4')
                  .ml-n16
                    DataPicker(:value="form.dateEnd" :min-date="false" @input="(v) => handleDatepickerChange(v, idx, 'dateEnd')")
...
export default {
  ...
  methods: {
    ...
    handleDatepickerChange(date, idx, key) {
      this.forms[idx][key] = moment(date).toDate()
    },
  },
}
</script>
