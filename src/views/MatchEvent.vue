<template>
  <Preloader fullPage v-if="!event.id" />
  <div class="d-flex create-event" v-else>
    <left-menu>
      ...
      <menu-filtering class="create-event__filter" @filter="sortAlphabetCurrent = $event">
        <template #left>
    ...
    <div class="container">
      <Header hideSearch />
      <bottom-header class="create-event__bottom-header" btnText="Добавить событие" :title="matchTypes">
        <template #icon>
        ...
        <template #btns>
          <div class="create-event__header-btns">
            <base-button
              class="create-event__header-btn"
              label="Редактировать"
              backgroundColor="#525BBA"
              @click="$router.push({ path: '/create-event', query: { edit: eventId } })"
            >
              <template #icon-left>
                <svg width="15" height="17" viewBox="0 0 15 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M3.80122 13.3013C3.80122 12.7898 3.79317 12.3453 3.80659 11.9009C3.81017 11.7936 3.85577 11.6594 3.9291 11.5852C5.51014 9.99076 7.09834 8.40346 8.68922 6.81079C9.20967 7.35718 9.70867 7.88121 10.2407 8.43833C10.1853 8.49825 10.0825 8.61718 9.97158 8.72897C8.58369 10.1204 7.18687 11.5029 5.81151 12.906C5.52892 13.194 5.24902 13.3514 4.83945 13.3084C4.51752 13.2745 4.19022 13.3013 3.80122 13.3013Z"
                    fill="white"
                  />
                  <path
                    d="M9.32617 6.10341C9.54437 5.90757 9.78671 5.63572 10.0827 5.44971C10.2267 5.3594 10.5352 5.34956 10.6586 5.44524C11.0199 5.72783 11.3275 6.07837 11.6503 6.40746C11.8694 6.63102 11.8381 6.85459 11.6307 7.07368C11.4062 7.31155 11.188 7.55568 10.9868 7.77567C10.427 7.21229 9.91012 6.69094 9.32617 6.10341Z"
                    fill="white"
                  />
                </svg>
              </template>
            ...
            <base-button class="create-event__header-btn" label="Напомнить о событии">
              <template #icon-left>
                <img src="@/assets/images/svg/icon-bell.svg" alt="bell" class="create-event__header-btn-icon" />
              </template>
            </base-button>
          </div>
        </template>
      ...
      <div class="create-event__date-address">
        ...
        <div class="create-event__date-address-elem">
          ...
          <p v-if="event.match">{{ event.match.location }}</p>
        </div>
      </div>
      ...
      <div class="create-event__route-block">
        <route-block v-if="routes.length" :routes="routes" />
      </div>

      <div class="create-event__group">
        <stages-preparation
          style="width: 49%"
          :items="preparationStages"
          :matchesId="matchesId"
          v-if="preparationStages.length"
        />
    ...
    <modal-player :visible="modalPlayer" @close="modalPlayer = false" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getEvent() {
      try {
        const response = await this.$events.getEvent(this.eventId)
        this.matchType = response.title
        this.routes = response.match.route.waypoints
        this.event = response

        this.eventDate = this.$moment(response?.recurrences.match(/(\d{8}T\d{6}Z)/g)[0]).format()
        this.eventDateF = this.$moment(response?.recurrences.match(/(\d{8}T\d{6}Z)/g)[0]).format('DD/MM/YYYY')

        this.timeStart = this.$moment(this.eventDate).format('HH:mm')
        this.timeEnd = this.$moment(this.eventDate).add(response.duration, 'minutes').format('HH:mm')
      } catch (e) {
        console.log(e)
      }
    },
    async getMatchesId() {
      const response = await this.$events.getAllMatches(this.eventId)
      if (response.length) {
        this.matchesId = response[0].id
      }
    },
    async getPreparationStages() {
      try {
        const response = await this.$events.getPreparationStages(this.eventId, this.matchesId)
        this.preparationStages = response
      } catch (e) {
        console.log(e)
      }
    },
